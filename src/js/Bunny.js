...
export class Bunny {
  ...
  constructor() {
    const scope = this;
    scope.model = new THREE.Group();
    const bunny = ModelManager.create("rabbit");
    bunny.castShadow = true;
    bunny.receiveShadow = true;
    bunny.scale.set(.5, .5, .5);
    scope.model.add(bunny);
    scope.mixer = new THREE.AnimationMixer(bunny);
    scope.jumpClip = ModelManager.getClip(bunny, "jump");
    scope.jumpAction = scope.mixer.clipAction(scope.jumpClip);
    scope.jumpAction.stop();
    scope.jumpUnitsPerSecond = 60;
